...
const DB={
    ...
    terbaru:p=>api(`${API_D}/new?page=${p}`),
    search:(q,p)=>api(`${API_D}/search?q=${encodeURIComponent(q)}&result=10&page=${p}`),
    detail:id=>api(`${API_D}/detail/${id}`),
    stream:(dramaId,episodeIndex)=>api(`${API_D}/stream?dramaId=${encodeURIComponent(dramaId)}&episodeIndex=${encodeURIComponent(episodeIndex)}`)
};
...
function getEpNum(ep,i){return gv(ep,'episode','number','episode_number','ep_number','ep','sort','order','section','index','episode_index')||(i+1);}
...
function getEpIndex(ep,i){const idx=gv(ep,'episode_index','index','episode','number','episode_number','ep_number','ep');const n=parseInt(idx,10);return Number.isNaN(n)?i:Math.max(0,n);} 
...
function card(d,s){
    ...
    return`<div class="d-card" onclick="openDrama('${escA(id)}','${escA(s)}')">
        <div class="c-poster">
            ...
            <div class="c-badge">
                <span class="${s==='melolo'?'b-src-m':'b-src-f'}">${s==='melolo'?'MLO':(s==='flickreels'?'FLK':'DBX')}</span>
...
function heroHTML(items,s){
    ...
    return`<section class="hero">
        ...
        <div class="hero-content" id="hC">
            <div class="hero-badges"><span class="h-badge red"><i class="fas fa-fire"></i> Populer</span><span class="h-badge blue">${s==='melolo'?'Melolo':(s==='flickreels'?'FlickReels':'DramaBox')}</span></div>
...
async function renderHome(){
    ...
    else if(src==='flickreels'){const[a,b]=await Promise.all([FL.trending(),FL.home(1)]);pop=findDramas(a);home=findDramas(b);}
    else{const[a,b]=await Promise.all([DB.populer(1),DB.home(1)]);pop=findDramas(a);home=findDramas(b);}
...
async function renderPop(p){
    ...
    let list=[];
    if(src==='melolo')list=findDramas(await ML.populer(pg));
    else if(src==='flickreels')list=findDramas(await FL.trending());
    else list=findDramas(await DB.populer(pg));
...
async function renderBrowse(p){
    ...
    let list=[];
    if(src==='melolo')list=findDramas(await ML.home(p));
    else if(src==='flickreels')list=findDramas(await FL.home(p));
    else list=findDramas(await DB.terbaru(p));
    list.forEach(d=>cachePoster(d));
...
async function renderSearch(q,p){
    ...
    let list=[];
    if(src==='melolo')list=findDramas(await ML.search(q,pg));
    else if(src==='flickreels')list=findDramas(await FL.search(q));
    else list=findDramas(await DB.search(q,pg));
    list.forEach(d=>cachePoster(d));
    const more=list.length>=8;
    let html=`<section class="section" style="padding-top:22px"><div class="sr-h"><h2>Hasil: <span>"${esc(q)}"</span></h2><p>${list.length} drama</p></div>${list.length?`<div class="d-grid" style="margin-top:16px">${list.map(d=>card(d,src)).join('')}</div>${(src==='melolo'||src==='dramabox')?pagHTML(pg,more,'renderSP'):''}`:emptyEl(`Tidak ditemukan "${esc(q)}"`)} </section>`;
    html+=footerEl();main.innerHTML=html;window.scrollTo({top:0,behavior:'smooth'});
}
...
async function openDrama(id,s){
    ...
    const data=s==='melolo'?await ML.detail(id):(s==='flickreels'?await FL.detail(id):await DB.detail(id));
    ...
    inner.innerHTML=`
            ...
            <div class="mod-pa">
                ...
                <div class="mod-ta">
                    ...
                    <div class="meta-r">
                        ...
                        ${lg?`<span><i class="fas fa-globe"></i> ${esc(lg)}</span>`:''}
                        <span style="color:var(--blue)">${s==='melolo'?'Melolo':(s==='flickreels'?'FlickReels':'DramaBox')}</span>
                    </div>
                </div>
            </div>
        </div>
        <div class="mod-body">
            ...
            <div class="info-grid">
                ...
                <div class="info-item"><div class="lbl">Sumber</div><div class="val">${s==='melolo'?'🎬 Melolo':(s==='flickreels'?'🎞️ FlickReels':'📦 DramaBox')}</div></div>
            </div>
            <div class="ep-sec">
                <div class="ep-h">
                ...
                ${eps.length>0
                    ?`<div class="ep-grid">${eps.map((ep,i)=>epCard(ep,i,title,s,id)).join('')}</div>`
...
function epCard(ep,i,dTitle,s,dramaId){
    const num=getEpNum(ep,i),epIndex=getEpIndex(ep,i),rawThumb=getEpThumb(ep,num),dur=ep.duration,vidId=getEpVid(ep),url=getEpUrl(ep);
    const durStr=dur?fmtDur(dur):'';
    let oc;
    if(s==='melolo'&&vidId)oc=`streamML('${escA(vidId)}','${escA(dTitle)} - Episode ${num}')`;
    else if(s==='dramabox')oc=`streamDB('${escA(dramaId)}',${epIndex},'${escA(dTitle)} - Episode ${num}')`;
...
async function streamDB(dramaId,episodeIndex,title){
    if(!dramaId||dramaId==='undefined'){toast('ID tidak valid');return;}
    toast('Memuat video...');
    const data=await DB.stream(dramaId,episodeIndex);
...
function playVid(url,title){
    ...
    document.getElementById('plInfo').textContent=`Sumber: ${src==='melolo'?'Melolo':(src==='flickreels'?'FlickReels':'DramaBox')}`;
